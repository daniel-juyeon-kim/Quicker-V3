## 2차 부하 테스트: 로드밸런서로 병렬 EC2 인스턴스를 구성해 CPU 사용률 20% 축소, TPS 4배 증가

> ##### 2025.08.28 ~ 2025.08.28 (1일)

2차 부하 테스트는 [1차 부하 테스트](<1차 부하 테스트: 인덱스로 RDS CPU 사용률 60% 감소, 최대 TPS 9배 증가.md#부하-테스트-환경>)보다 **가상 사용자를 100 → 1,000으로 늘렸습니다.**
